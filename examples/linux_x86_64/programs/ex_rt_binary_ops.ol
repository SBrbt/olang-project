...
extern i32 main() {
  let a<b32> @stack<32>(65280b32);
  let b<b32> @stack<32>(4080b32);

  // Bitwise AND: 65280 & 4080 = 3840
  let and_r<b32> @stack<32>(load<a> & load<b>);
  if (load<and_r> != 3840b32) { return 1; }

  // Bitwise OR: 65280 | 4080 = 65520
  let or_r<b32> @stack<32>(load<a> | load<b>);
  if (load<or_r> != 65520b32) { return 2; }

  // Bitwise XOR: 65280 ^ 4080 = 61680
  let xor_r<b32> @stack<32>(load<a> ^ load<b>);
  if (load<xor_r> != 61680b32) { return 3; }

  // Left shift
  let shl_r<b32> @stack<32>(1b32 << 8b32);
  if (load<shl_r> != 256b32) { return 5; }

  // Right shift
  let shr_r<b32> @stack<32>(256b32 >> 4b32);
  if (load<shr_r> != 16b32) { return 6; }

  // Arithmetic on bN
  let c<b32> @stack<32>(100b32);
  let d<b32> @stack<32>(30b32);
